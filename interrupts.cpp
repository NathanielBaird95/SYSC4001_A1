...
int main(int argc, char **argv)
{
    ...
    int context_save_ms = 10;      // professor said use 10 (try 20, 30 in experiments)
    int syscall_overhead_ms = 1;   // SYSCALL start I/O step
    int scheduler_overhead_ms = 1; // scheduler picks/dispatches process
    int return_overhead_ms = 1;    // IRET / return to user mode
    int isr_chunk_ms = 40;         // each ISR activity chunk, can vary (40–200)
    ...
    while (std::getline(input_file, trace))
    {
        ...
        else if (activity == "END_IO")
        {
            ...
            while (remaining > 0)
            {
                int chunk = std::min(isr_chunk_ms, remaining);
